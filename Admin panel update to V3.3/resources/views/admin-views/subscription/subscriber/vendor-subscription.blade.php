            ...
            <div class="card-body pt-0 px-0">
                ...
                <div class="btn--container justify-content-end mt-3">
                    @if ( $store?->store_sub_update_application?->is_canceled == 0 && $store?->store_sub_update_application?->status == 1  )
                        @php
                            $daysRemaining = Carbon\Carbon::now()->diffInDays($store?->store_sub_update_application?->expiry_date_parsed->format('Y-m-d'), false);
                            $cancelMessage = translate('If_you_cancel_the_subscription,_after_') . ' ' . $daysRemaining . ' ' . translate('days_the_vendor_will_no_longer_be_able_to_run_the_business_before_subscribe_a_new_plan.');
                        @endphp
                        <button type="button"  data-url="{{route('admin.business-settings.subscriptionackage.cancelSubscription',$store?->id)}}" data-message="{{$cancelMessage}}"

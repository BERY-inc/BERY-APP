...
class WalletController extends Controller
{
    ...
    public function fund_transfer(Request $request)
    {
        ...
        if (!$recipient) {
            return response()->json(['errors' => [['code' => 'recipient', 'message' => 'Recipient not found']]], 404);
        }

        if ($sender->id == $recipient->id) {
            return response()->json(['errors' => [['code' => 'recipient', 'message' => 'You cannot send money to yourself']]], 400);
        }

        if ($sender->wallet_balance < $request->amount) {
            return response()->json(['errors' => [['code' => 'insufficient_balance', 'message' => 'Insufficient wallet balance']]], 403);
        }
        ...
        try {
            ...
            $senderTrx->transaction_id = Str::uuid();
            $senderTrx->transaction_type = 'fund_transfer';
            $senderTrx->debit = $request->amount;
            $senderTrx->balance = $sender->wallet_balance;
            $senderTrx->reference = $recipient->f_name . ' ' . $recipient->l_name . ' (' . $recipient->phone . ')';
            ...
            $recipientTrx = new WalletTransaction();
            $recipientTrx->user_id = $recipient->id;
            $recipientTrx->transaction_id = Str::uuid();
            $recipientTrx->transaction_type = 'fund_transfer_received';
            $recipientTrx->credit = $request->amount;
            $recipientTrx->balance = $recipient->wallet_balance;
            $recipientTrx->reference = $sender->f_name . ' ' . $sender->l_name . ' (' . $sender->phone . ')';
            $recipientTrx->created_at = now();
            $recipientTrx->updated_at = now();
            $recipientTrx->save();

            DB::commit();

            // Send Notifications
            $fcm_token = $recipient->cm_firebase_token;
            if ($fcm_token) {
                $data = [
                    'title' => 'Fund Received',
                    'description' => 'You have received ' . Helpers::format_currency($request->amount) . ' from ' . $sender->f_name,
                    'order_id' => '',
                    'image' => '',
                    'type' => 'wallet'
                ];
            ...
            DB::rollBack();
            return response()->json(['errors' => [['code' => 'error', 'message' => 'Transaction failed: ' . $e->getMessage()]]], 500);
        }
    }
}

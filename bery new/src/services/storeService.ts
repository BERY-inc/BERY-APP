...
const storeService = {
  ...
  getLatestStores: async (options?: { limit?: number; offset?: number }): Promise<Store[]> => {
    try {
      const params: any = {
        limit: options?.limit ?? 20,
        offset: options?.offset ?? 0,
  ...
  getPopularStores: async (): Promise<Store[]> => {
    try {
      const response = await apiClient.get<{ stores: Store[] }>('\/api\/v1\/stores\/popular');
      return response.data?.stores ?? [];
    } catch (error) {
